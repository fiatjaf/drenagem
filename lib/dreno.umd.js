...
var toInteger = function (value) {
	...
	return index$2(value) * floor$1(abs$1(value));
};
...
var shim$5 = function (dest, src/*, …srcn*/) {
	...
	for (i = 1; i < l; ++i) {
		src = arguments$1[i];
		index$10(src).forEach(assign);
	}
	if (error !== undefined) { throw error; }
	return dest;
};

var index$8 = isImplemented$6()
	? Object.assign
	: shim$5;
...
d = module.exports = function (dscr, value/*, options*/) {
	...
	if (dscr == null) {
		...
		c = index$12.call(dscr, 'c');
		e = index$12.call(dscr, 'e');
		w = index$12.call(dscr, 'w');
	}

	desc = { value: value, configurable: c, enumerable: e, writable: w };
	return !options ? desc : index$8(normalizeOptions(options), desc);
};
...
var generateName = (function () {
	...
	return function (desc) {
		...
		created[desc] = true;
		name = '@@' + desc;
		defineProperty$2(objPrototype, name, index$6.gs(null, function (value) {
			// For IE11 issue see:
			...
			defineProperty$2(this, name, index$6(value));
...
var polyfill$2 = SymbolPolyfill = function Symbol(description) {
	...
	return defineProperties(symbol, {
		__description__: index$6('', description),
		__name__: index$6('', generateName(description))
	});
};
defineProperties(SymbolPolyfill, {
	for: index$6(function (key) {
		if (globalSymbols[key]) { return globalSymbols[key]; }
		return (globalSymbols[key] = SymbolPolyfill(String(key)));
	}),
	keyFor: index$6(function (s) {
		var key;
		validateSymbol(s);
	...
	hasInstance: index$6('', (NativeSymbol && NativeSymbol.hasInstance) || SymbolPolyfill('hasInstance')),
	isConcatSpreadable: index$6('', (NativeSymbol && NativeSymbol.isConcatSpreadable) ||
		SymbolPolyfill('isConcatSpreadable')),
	iterator: index$6('', (NativeSymbol && NativeSymbol.iterator) || SymbolPolyfill('iterator')),
	match: index$6('', (NativeSymbol && NativeSymbol.match) || SymbolPolyfill('match')),
	replace: index$6('', (NativeSymbol && NativeSymbol.replace) || SymbolPolyfill('replace')),
	search: index$6('', (NativeSymbol && NativeSymbol.search) || SymbolPolyfill('search')),
	species: index$6('', (NativeSymbol && NativeSymbol.species) || SymbolPolyfill('species')),
	split: index$6('', (NativeSymbol && NativeSymbol.split) || SymbolPolyfill('split')),
	toPrimitive: index$6('', (NativeSymbol && NativeSymbol.toPrimitive) || SymbolPolyfill('toPrimitive')),
	toStringTag: index$6('', (NativeSymbol && NativeSymbol.toStringTag) || SymbolPolyfill('toStringTag')),
	unscopables: index$6('', (NativeSymbol && NativeSymbol.unscopables) || SymbolPolyfill('unscopables'))
});

// Internal tweaks for real symbol producer
defineProperties(HiddenSymbol.prototype, {
	constructor: index$6(SymbolPolyfill),
	toString: index$6('', function () { return this.__name__; })
});

// Proper implementation of methods exposed on Symbol.prototype
// They won't be accessible on produced symbol instances as they derive from HiddenSymbol.prototype
defineProperties(SymbolPolyfill.prototype, {
	toString: index$6(function () { return 'Symbol (' + validateSymbol(this).__description__ + ')'; }),
	valueOf: index$6(function () { return validateSymbol(this); })
});
defineProperty$2(SymbolPolyfill.prototype, SymbolPolyfill.toPrimitive, index$6('', function () {
	var symbol = validateSymbol(this);
	if (typeof symbol === 'symbol') { return symbol; }
	return symbol.toString();
}));
defineProperty$2(SymbolPolyfill.prototype, SymbolPolyfill.toStringTag, index$6('c', 'Symbol'));

// Proper implementaton of toPrimitive and toStringTag for returned symbol instances
defineProperty$2(HiddenSymbol.prototype, SymbolPolyfill.toStringTag,
	index$6('c', SymbolPolyfill.prototype[SymbolPolyfill.toStringTag]));
...
defineProperty$2(HiddenSymbol.prototype, SymbolPolyfill.toPrimitive,
	index$6('c', SymbolPolyfill.prototype[SymbolPolyfill.toPrimitive]));

var index$16 = isImplemented$12() ? Symbol : polyfill$2;
...
var iteratorSymbol = index$16.iterator;
...
var iteratorSymbol$2 = index$16.iterator;
...
var copy = function (obj/*, propertyNames, options*/) {
	...
	var result = {};
	if (propertyNames) {
		index$20(propertyNames, function (propertyName) {
...
var index$18 = Iterator = function (list, context) {
	if (!(this instanceof Iterator)) { return new Iterator(list, context); }
	defineProperties$1(this, {
		__list__: index$6('w', validValue(list)),
		__context__: index$6('w', context),
		__nextIndex__: index$6('w', 0)
	});
	if (!context) { return; }
...
defineProperties$1(Iterator.prototype, index$8({
	constructor: index$6(Iterator),
	_next: index$6(function () {
		var i;
	...
	next: index$6(function () { return this._createResult(this._next()); }),
	_createResult: index$6(function (i) {
		if (i === undefined) { return { done: true, value: undefined }; }
		return { done: false, value: this._resolve(i) };
	}),
	_resolve: index$6(function (i) { return this.__list__[i]; }),
	_unBind: index$6(function () {
		this.__list__ = null;
	...
	toString: index$6(function () { return '[object Iterator]'; })
}, autoBind({
	_onAdd: index$6(function (index) {
		if (index >= this.__nextIndex__) { return; }
		++this.__nextIndex__;
		if (!this.__redo__) {
			defineProperty$3(this, '__redo__', index$6('c', [index]));
			return;
		}
	...
	_onDelete: index$6(function (index) {
		var i;
		if (index >= this.__nextIndex__) { return; }
		--this.__nextIndex__;
	...
	_onClear: index$6(function () {
		if (this.__redo__) { clear.call(this.__redo__); }
		this.__nextIndex__ = 0;
	})
})));

defineProperty$3(Iterator.prototype, index$16.iterator, index$6(function () {
	return this;
}));
defineProperty$3(Iterator.prototype, index$16.toStringTag, index$6('', 'Iterator'));
...
ArrayIterator = module.exports = function (arr, kind) {
	...
	index$18.call(this, arr);
	if (!kind) { kind = 'value'; }
	else if (index$12.call(kind, 'key+value')) { kind = 'key+value'; }
	else if (index$12.call(kind, 'key')) { kind = 'key'; }
	else { kind = 'value'; }
	defineProperty(this, '__kind__', index$6('', kind));
};
if (index$4) { index$4(ArrayIterator, index$18); }

ArrayIterator.prototype = Object.create(index$18.prototype, {
	constructor: index$6(ArrayIterator),
	_resolve: index$6(function (i) {
		if (this.__kind__ === 'value') { return this.__list__[i]; }
		if (this.__kind__ === 'key+value') { return [i, this.__list__[i]]; }
		return i;
	}),
	toString: index$6(function () { return '[object Array Iterator]'; })
});
});
...
StringIterator = module.exports = function (str) {
	...
	str = String(str);
	index$18.call(this, str);
	defineProperty(this, '__length__', index$6('', str.length));

};
if (index$4) { index$4(StringIterator, index$18); }

StringIterator.prototype = Object.create(index$18.prototype, {
	constructor: index$6(StringIterator),
	_next: index$6(function () {
		if (!this.__list__) { return; }
	...
	toString: index$6(function () { return '[object String Iterator]'; })
});
});

var iteratorSymbol$1 = index$16.iterator;
...
var toStringTagSymbol = index$16.toStringTag
...
SetIterator = module.exports = function (set, kind) {
	...
	index$18.call(this, set.__setData__, set);
	if (!kind) { kind = 'value'; }
	else if (index$12.call(kind, 'key+value')) { kind = 'key+value'; }
	else { kind = 'value'; }
	defineProperty(this, '__kind__', index$6('', kind));
};
if (index$4) { index$4(SetIterator, index$18); }

SetIterator.prototype = Object.create(index$18.prototype, {
	constructor: index$6(SetIterator),
	_resolve: index$6(function (i) {
		if (this.__kind__ === 'value') { return this.__list__[i]; }
		return [this.__list__[i], this.__list__[i]];
	}),
	toString: index$6(function () { return '[object Set Iterator]'; })
});
defineProperty(SetIterator.prototype, toStringTagSymbol, index$6('c', 'Set Iterator'));
});
...
var polyfill = SetPoly = function Set(/*iterable*/) {
	...
	if (isNativeImplemented && index$4) { self = index$4(new NativeSet(), getPrototypeOf(this)); }
	else { self = this; }
	if (iterable$$1 != null) { iterator(iterable$$1); }
	defineProperty(self, '__setData__', index$6('c', []));
...
if (isNativeImplemented) {
	if (index$4) { index$4(SetPoly, NativeSet); }
	SetPoly.prototype = Object.create(NativeSet.prototype, { constructor: index$6(SetPoly) });
}

index$14(Object.defineProperties(SetPoly.prototype, {
	add: index$6(function (value) {
		if (this.has(value)) { return this; }
		this.emit('_add', this.__setData__.push(value) - 1, value);
		return this;
	}),
	clear: index$6(function () {
		if (!this.__setData__.length) { return; }
		clear.call(this.__setData__);
		this.emit('_clear');
	}),
	delete: index$6(function (value) {
	...
	entries: index$6(function () { return new iterator$1(this, 'key+value'); }),
	forEach: index$6(function (cb/*, thisArg*/) {
		var this$1 = this;
	...
	has: index$6(function (value) {
		return (eIndexOf.call(this.__setData__, value) !== -1);
	}),
	keys: index$6(getValues = function () { return this.values(); }),
	size: index$6.gs(function () { return this.__setData__.length; }),
	values: index$6(function () { return new iterator$1(this); }),
	toString: index$6(function () { return '[object Set]'; })
}));
defineProperty(SetPoly.prototype, index$16.iterator, index$6(getValues));
defineProperty(SetPoly.prototype, index$16.toStringTag, index$6('c', 'Set'));

var index$1 = isImplemented() ? Set : polyfill;
...
track.withValue = function (value) {
  return function (e) {
    eventStreams['__global__'].shamefullySendNext({e: e, value: value, element: e.currentTarget});
  }
};
...
function select (selector, tag) {
  ...
  ee.events = function (selectedType) {
    return ee
      ...
      .map(function (meta) { return meta.e; })
  };
